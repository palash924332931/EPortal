...
declare @tablename as varchar(255) = 'DIMOutlet'
...
insert into dbo.QCLog ([Status],[Date],[Message]) VALUES('Info',GETDATE ( ) ,  'DIMOutlet column Check Start.')
...
select @result=cnt,@clmName=tabcolname  from @maintable where id=@mincnt
 ...
 begin
 insert into dbo.QCLog ([Status],[Date],[Message]) VALUES('Error',GETDATE ( ) ,  'There is no data in the column '+@clmName + ' of tabel DIMOutlet' )
 end
 set @mincnt = @mincnt +1 
end

insert into dbo.QCLog ([Status],[Date],[Message]) VALUES('Info',GETDATE ( ) ,  'DIMOutlet column Check End.')