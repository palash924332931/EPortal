CREATE PROCEDURE [dbo].[CombineMultipleMarketBasesForAll_test]
 
AS
BEGIN
	SET NOCOUNT ON;
	----REMOVE DUPLICATE RECORDS FROM MDefG4----
	with CTE_DUP as
	(
	...
	join MDefG4 M on C.Pack = M.Pack and C.MarketDefinitionId = M.MarketDefinitionId and C.PFC = M.PFC AND C.GroupNumber = M.GroupNumber 
	...
	delete C from MDefG4 C 
	inner join #t M on C.Pack = M.Pack and C.MarketDefinitionId = M.MarketDefinitionId and C.PFC = M.PFC --AND C.GroupNumber = M.GroupNumber and C.ATC4 = M.ATC4 and C.NEC4 = isnull(M.NEC4,'') and C.Manufacturer = isnull(M.Manufacturer,'')
	--and id not in (198252)

	-----INSERT COMIBNED MARKET BASES FOR COMMON PACKS--------
	insert into MDefG4 select * from #aggregatedMarketBase

END

--select * from MDefG4 WHERE marketbase like '%,%'
--select * from marketdefinitionpacks WHERE marketbase like '%,%'

--[dbo].[CombineMultipleMarketBasesForAll]

--select count(*) from MDefG4
--SELECT COUNT(*) FROM marketdefinitionpacks
--SELECT COUNT(*) FROM marketdefinitionpacks_backup2