...
BEGIN
	...
	SET @RecipientListFor2DaysRemainder =  STUFF((SELECT ';' + email FROM [User] WHERE IsActive = 1 AND DATEDIFF(DAY, PasswordCreatedDate, GETDATE()) = (@PasswordExpirationAge - 1) FOR XML PATH('')),1,1,'')
	SET @RecipientListFor1DaysRemainder =  STUFF((SELECT ';' + email FROM [User] WHERE IsActive = 1 AND DATEDIFF(DAY, PasswordCreatedDate, GETDATE()) = @PasswordExpirationAge FOR XML PATH('')),1,1,'')
